...
let URL = (https://teachablemachine.withgoogle.com/models/6kPN3WDpz/");
    ...
    if (prob > 0.8 && !explosionActive) {
        ...
        switch(poseNumber) {
            ...
            case '2':
                ...
                break;
            case '3':
                if ((time >= 8.0 && time <= 9.0 && !poseState.firstWindowTriggered) ||
                    (time >= 9.0 && time <= 10.0 && !poseState.secondWindowTriggered)) {
                    if (time <= 9.0) {
                ...
                break;
            case '4':
                if (time >= 11.0 && time <= 13.0 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
...
async function playInstructionVideo() {
    ...
    if (model) {
        ...
        console.log("https://teachablemachine.withgoogle.com/models/6kPN3WDpz//");
    }
}
